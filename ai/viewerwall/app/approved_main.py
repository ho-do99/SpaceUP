...
def _significant_grid_coordinates(polygon, horizontal, vertical, image_shape):
    ...
    # A SPA contour often contains one- or two-cell teeth around a wall join.
    # Do not let those short runs create a new wall coordinate.  The threshold
    # is deliberately relative to the drawing size so the same rule also works
    # for plans exported at a different resolution.
    minimum_edge = max(18.0, min(42.0, min(image_shape) * 0.008))
    x_values = []
    y_values = []
    supported_x = []
    supported_y = []
    ...
    for index in range(count):
        ...
        if abs(dx) >= minimum_edge and abs(dx) >= abs(dy) * 2.0:
            ...
            snapped = _nearest_segment_coordinate(
                horizontal, coordinate, start[0], end[0], snap_tolerance
            )
            y_values.append(snapped)
            supported_y.extend([float(start[1]), float(end[1]), snapped])
            ...
            x_values.append(snapped)
            supported_x.extend([float(start[0]), float(end[0]), snapped])

    xs = [int(point[0]) for point in simplified]
    ys = [int(point[1]) for point in simplified]
    # Prefer extrema that are backed by a continuous boundary run.  Raw
    # min/max vertices are used only as a fallback for a genuinely tiny room;
    # previously they reintroduced every short SPA protrusion into the grid.
    if len(x_values) >= 2 and supported_x:
        x_values.extend([min(supported_x), max(supported_x)])
    else:
        x_values.extend([min(xs), max(xs)])
    if len(y_values) >= 2 and supported_y:
        y_values.extend([min(supported_y), max(supported_y)])
    else:
