...
export default function ContractorVisitChangeRejectDialog({ open, onClose, onConfirm }: ContractorVisitChangeRejectDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0f172a]/60 px-4" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section role="dialog" aria-modal="true" aria-labelledby="visit-change-reject-title" className="w-full max-w-[361px] rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <h2 id="visit-change-reject-title" className="mt-3 text-center text-base font-bold text-[#1e293b]">변경 요청을 거절하시겠어요?</h2>
        <p className="mt-2 text-center text-xs leading-5 text-[#64748b]">거절하면 기존 방문 일정이 유지됩니다.</p>
        <div className="mt-5 grid grid-cols-2 gap-2">
          ...
          <button type="button" onClick={onConfirm} className="h-11 rounded-lg border border-[#ef4444] text-xs font-bold text-[#ef4444]">변경 요청 거절</button>
        </div>
      </section>
    </div>
  )
