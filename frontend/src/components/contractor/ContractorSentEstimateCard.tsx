  ...
  return (
    <article className="rounded-xl border border-[#e2e8f0] bg-white p-3 shadow-sm">
      <div className="flex min-w-0 items-start justify-between gap-2">
        <p className="min-w-0 break-all text-[13px] font-bold text-[#64748b]">{estimate.estimateId}</p>
        <ContractorEstimateLifecycleBadge status={status} />
      </div>
      <p className="mt-2 break-words text-[11px] leading-5 text-[#64748b]">{estimate.customerName} · {estimate.region} · {estimate.propertyType} {estimate.areaLabel}</p>
