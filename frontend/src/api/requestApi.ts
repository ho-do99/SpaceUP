...
export async function createRequest(input: RequestCreateInput) {
  const response = await apiRequest<ApiResponse<RequestResponse>, RequestCreateInput>({
    method: 'POST', url: '/api/requests', data: input, authenticated: true,
  })
  return unwrapApiResponse<RequestResponse>(response, '의뢰 생성에 실패했습니다.')
}
...
export async function updateRequest(requestId: number, input: RequestUpdateInput) {
  const response = await apiRequest<ApiResponse<RequestResponse>, RequestUpdateInput>({
    method: 'PATCH', url: `/api/requests/${requestId}`, data: input, authenticated: true,
  })
  return unwrapApiResponse<RequestResponse>(response, '의뢰 수정에 실패했습니다.')
}

export async function attachRequestImage(requestId: number, input: RequestImageInput) {
  const response = await apiRequest<ApiResponse<RequestImageResponse>, RequestImageInput>({
    method: 'POST', url: `/api/requests/${requestId}/images`, data: input, authenticated: true,
  })
  return unwrapApiResponse<RequestImageResponse>(response, '이미지 연결에 실패했습니다.')
}
