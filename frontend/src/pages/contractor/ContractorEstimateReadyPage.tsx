...
export default function ContractorEstimateReadyPage() {
  ...
  const { visitSchedule, visitStatus } = useContractorPortalFlow()
  ...
  return (
    <ContractorMobileShell>
      <ContractorAppBar title="견적서 작성" back />
      <main className="flex flex-1 flex-col px-4 pb-6 pt-4">
        <section className="rounded-xl border border-[#a7f3d0] bg-[#ecfdf5] p-4 text-[#047857]">
          <span aria-hidden="true" className="flex h-10 w-10 items-center justify-center rounded-full bg-white text-xl">✓</span>
          <h2 className="mt-3 text-base font-bold">현장 방문을 완료했습니다</h2>
          <p className="mt-1 text-xs leading-5">확인한 현장 정보로 견적 작성을 시작할 수 있습니다.</p>
        </section>
        <ContractorSectionCard className="mt-4" title="의뢰 정보">
          <p className="text-sm font-bold text-[#2563eb]">{request.requestId}</p>
          <p className="mt-2 text-xs text-[#64748b]">{request.customerName} · {request.property.propertyType} {request.property.areaLabel}</p>
          <p className="mt-1 break-words text-xs leading-5 text-[#64748b]">{request.property.address}</p>
        </ContractorSectionCard>
        <ContractorSectionCard className="mt-3" title="현장 방문 결과">
          <p className="text-xs text-[#64748b]">방문일 {schedule.date.replace(/-/g, '.')} · {schedule.time}</p>
