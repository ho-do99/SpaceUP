...
export default function ContractorEstimateSentPage() {
  ...
  const { visitStatus, estimateDraft, estimateStatus, estimateSubmission } = useContractorPortalFlow()
  ...
  if (!estimateDraft) return <Navigate to={`/contractor/requests/${request.requestId}/estimate`} replace />
  if (estimateStatus !== 'SUBMITTED' || !estimateSubmission) return <Navigate to={`/contractor/requests/${request.requestId}/estimate/preview`} replace />

  return (
    <ContractorMobileShell>
      <ContractorAppBar title="보낸 견적" />
      <main className="flex-1 overflow-y-auto px-4 pb-8 pt-4">
        <p className="text-xs leading-5 text-[#64748b]">작성한 견적이 전송 완료 상태로 반영되었습니다.</p>
        ...
        <ContractorSectionCard className="mt-4">
          <div className="flex items-center justify-between gap-3">
            <p className="text-[13px] font-bold text-[#2563eb]">{estimateSubmission.estimateNumber}</p>
            <span className="rounded-full bg-[#eff6ff] px-3 py-1 text-[10px] font-bold text-[#2563eb]">전송 완료</span>
          </div>
          <p className="mt-3 text-xs text-[#64748b]">광주 서구 ○○아파트 · {formatWon(calculateEstimateTotal(estimateDraft))}</p>
          <dl className="mt-3 space-y-2 border-t border-[#e2e8f0] pt-3">
            <ContractorEstimateInfoRow label="전송일">{estimateSubmission.submittedDate.replace(/-/g, '.')}</ContractorEstimateInfoRow>
            <ContractorEstimateInfoRow label="유효일">{estimateSubmission.validUntil.replace(/-/g, '.')}</ContractorEstimateInfoRow>
          </dl>
