...
async function searchAndSelect() {
  ...
  await waitFor(() => expect(search).toHaveBeenCalledWith({ keyword: '상무', page: 0, size: 20 }))
  fireEvent.click(await screen.findByRole('button', { name: /상무센트럴아파트/ }))
}

describe('ApartmentAddressSearchPage storage floor plan flow', () => {
  beforeEach(() => { search.mockReset(); create.mockReset().mockResolvedValue(77); createJob.mockReset().mockResolvedValue(88); sessionStorage.clear() })
  afterEach(cleanup)
  ...
  it('keeps manual area validation for an empty result', async () => {
    search.mockResolvedValue(page([])); renderPage()
    fireEvent.change(screen.getByLabelText('아파트명 또는 주소'), { target: { value: '없음' } }); fireEvent.click(screen.getByRole('button', { name: '아파트 검색' }))
